    ...
    <main class="main-content">
      ...
      <div class="order-tabs">
        <el-radio-group v-model="statusFilter" @change="loadOrders">
          ...
          <el-radio-button value="8">已取消</el-radio-button>
        </el-radio-group>
      </div>

      <div v-if="loading" class="empty-wrap" v-loading="true" style="min-height: 200px"></div>

      <div v-else-if="orderList.length === 0" class="empty-wrap">
...
const getTagType = (status) => ({ 0: 'warning', 1: 'primary', 2: 'success', 3: 'info', 8: 'danger' }[status] ?? 'info')
const getStatusBorderClass = (status) => ({ 0: 'border-warning', 1: 'border-primary', 2: 'border-success', 3: 'border-info', 8: 'border-danger' }[status] ?? '')
const getStatusText = (status) => ({ 0: '待支付', 1: '待发货', 2: '待收货', 3: '已完成', 8: '已取消' }[status] ?? '未知状态')
</script>
